...
Running a door with `WithMaxAttempts(1)` inside your own retry loop is the sanctioned way to avoid multiplying the two attempt counts (a 3-attempt door inside a 3-attempt loop is 9 requests). The door then makes no retry decision, so you make it. The two exported predicates are the door's own rule, so your loop and the built-in one classify identically:
