...
- `Drain` / `DrainClose`: drain a body for connection reuse (64 KB limit)
...
httpx retries transient failures, not budget expiry. `IsTransient` classifies `context.DeadlineExceeded` and `context.Canceled` as **non-transient**, while a connection reset, a DNS error, a `net.Error` timeout that is not a deadline, and a 429/5xx are transient. A caller's context deadline means "the budget is exhausted, stop"; the expiry of a bound over ONE attempt means "this attempt failed, try again". The same `context.DeadlineExceeded` value carries both meanings, so only the code that installed the bound can tell them apart — `AttemptTimeout(err)` is how it says so, and `IsTransient` honors that mark ahead of its context-error rejection.
...
- **Per-attempt bound anywhere else: mark it with `AttemptTimeout(err)`.** `http.Client.Timeout` and a transport `ResponseHeaderTimeout` both bound a single attempt, but net/http reports their expiry as an error that matches `context.DeadlineExceeded` (`net/http`'s internal timeout error does so deliberately), so `IsTransient` reads it as budget expiry and classifies it **terminal** — including under the `RetryRoundTripper`'s default policy. Wrap it in a `TransportConfig.CheckRetry` (or at the error site in a `Do` callback) to opt it back in. The same applies to a `context.WithTimeout` a caller derives itself inside `fn`; `WithAttemptTimeout` is that case done for you.
- **`GetBytes`** takes no per-attempt option: the door owns its request and has no callback to bound. Run it as one attempt inside `Do` — `WithMaxAttempts(1)` on the GET, `WithAttemptTimeout(d)` on the `Do` — which is also the composition that avoids multiplying two attempt budgets.

Under `NewRetryRoundTripper` / `NewRetryClient` the retry loop runs _inside_ `client.Do`, so `http.Client.Timeout` is not per-attempt at all: it caps the whole retry sequence and a slow attempt that trips it aborts the remaining retries, which is why `NewRetryClient` sets none. Put the per-attempt bound on the base transport (`ResponseHeaderTimeout`) and the total on the caller's context or `TransportConfig.MaxElapsedTime`. Neither the between-attempt `MaxElapsedTime` check nor an expired context can interrupt an attempt already stalled inside the base transport; only a transport-level timeout can.
